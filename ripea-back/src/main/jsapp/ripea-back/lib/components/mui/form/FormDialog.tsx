...
export const useFormDialog: UseFormDialogFn = (
    ...
    const {initOnChangeRequest: initOnChangeRequestProp, ...otherFormComponentProps} = defaultFormComponentProps ?? {};
    ...
    const buttonCallback = (value: any) => {
        if (value) {
            ...
            const submitPromise = isCustomSubmit ? customSubmit(formApiRef.current.getId(), formApiRef.current.getData()) : formApiRef.current.save();
            setLoading(true);
            submitPromise.then((value: any) => {
                if (isCustomSubmit) {
