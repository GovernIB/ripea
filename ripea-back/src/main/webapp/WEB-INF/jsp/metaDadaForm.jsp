...
<body>
	<form:form action="." method="post" cssClass="form-horizontal" modelAttribute="metaDadaCommand">
		...
		<rip:inputSelect name="tipus" textKey="metadada.form.camp.tipus" optionItems="${tipus}" optionValueAttribute="value" optionTextKeyAttribute="text" disabled="${bloquejarCamps || (metaDadaCommand.id!=null && existContingut)}"/>
		...
		<div id="modal-botons">
			<c:if test="${!consultar}">
				<button type="submit" class="btn btn-success" <c:if test="${bloquejarCamps}">disabled</c:if>><span class="fa fa-save"></span>
					<c:choose>
						<c:when test="${empty metaDadaCommand.id}"><spring:message code="comu.boto.crear"/></c:when>
