	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("button[name=refrescar]").click(function() {
            	...
            	carregaMonitorWithAnimation();
			});
			carregaMonitorWithAnimation();
		});

		// show loading icon so user can see new content is being loaded
		function carregaMonitorWithAnimation() {
			if (typeof loaded === 'undefined') {
				carregaMonitor();
			} else {
				$("#monitor_contens").html("<div style='text-align: center; margin-bottom: 200px; margin-top: 200px;'><span class='fa fa-circle-o-notch fa-spin fa-3x' style='color: #666666;'/></div>");
			    setTimeout(function(){
			    	carregaMonitor();
			    }, 50);				
			}
		    ...
		    $.ajax({
		        ...
		        success: function(data){
		            ...
		            content +=  '<div id="mesures_monitor" class="top-buffer">' +
		                        ...
		                        '</div>';
		                          ...
		                          content += '<div role="tabpanel" class="tab-pane" id="tasques">';
		       		            content += '<div id="mesures_monitor" class="top-buffer">' +
		       			                        '<table class="table-monitor table table-striped table-bordered dataTable">' +
		       			                        '<thead><tr>' +
		       			                        '<th class="" style="width: 30%"><spring:message code="monitor.tasques.tasca"/></th>' +
		    			...
		    			if (/#sistema/.test(window.location.href)) {
		    				$('.nav-tabs a[href$="#sistema"]').trigger('click');	
		    			} else if (/#fils/.test(window.location.href)){
		    				$('.nav-tabs a[href$="#fils"]').trigger('click');
