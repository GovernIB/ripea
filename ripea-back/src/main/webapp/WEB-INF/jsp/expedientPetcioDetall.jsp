			...
			<div class="panel panel-default">
				...
				<c:choose>
					<c:when test="${not empty registre.interessats}">
						...
						<tbody>
							<c:forEach var="intFis" items="${registre.interessats}" varStatus="status">
								<tr <c:if test="${status.index%2 == 0}">class="odd"</c:if>>
									<td>
										<spring:message code="peticio.registre.interessat.tipus.enum.${intFis.tipus}"/>
									</td>
									<td>${intFis.documentTipus}: ${intFis.documentNumero}</td>
									<c:choose>
										<c:when test="${intFis.tipus == 'PERSONA_FISICA'}">
											<td>${intFis.nom} ${intFis.llinatge1} ${intFis.llinatge2}</td>
										</c:when>
										<c:otherwise>
											<td>${intFis.raoSocial}</td>
										</c:otherwise>
									</c:choose>
									<td>
								...
								<tr class="collapse detall" id="detalls_${status.index}_resum">
									<td colspan="4">
										<div class="row">
											<div class="col-xs-6">
												<dl class="dl-horizontal">
													<dt><spring:message code="interessat.form.camp.pais"/></dt><dd>${intFis.pais}</dd>
													<dt><spring:message code="interessat.form.camp.provincia"/></dt><dd>${intFis.provincia}</dd>
													<dt><spring:message code="interessat.form.camp.municipi"/></dt><dd>${intFis.municipi}</dd>
													<dt><spring:message code="interessat.form.camp.adresa"/></dt><dd>${intFis.adresa}</dd>
													<dt><spring:message code="interessat.form.camp.codiPostal"/></dt><dd>${intFis.cp}</dd>
												</dl>
											</div>
											<div class="col-xs-6">
												<dl class="dl-horizontal">
													...
													<dt><spring:message code="registre.interessat.detalls.camp.canalPreferent"/></dt><dd><c:if test="${not empty intFis.canal}"><spring:message code="registre.interessat.detalls.camp.canalPreferent.${intFis.canal}"/></c:if></dd>
							...
							<tbody>
								<c:forEach var="annex" items="${registre.annexos}" varStatus="status">
									<tr title="<spring:message code="registre.annex.detalls.previsualitzar"/>" <c:choose><c:when test="${annex.tipusMime == 'application/pdf' }">onclick="showViewer(event, ${annex.id}, '${annex.observacions}', '${annex.ntiFechaCaptura}', '${annex.ntiOrigen}')"</c:when><c:otherwise>class="invalid-format"</c:otherwise></c:choose>>
										<td>${annex.titol} <c:if test="${!annex.validacioFirmaCorrecte}"><span class="fa fa-exclamation-triangle text-danger" title="<spring:message code="contingut.icona.estat.invalid.origen" arguments="${annex.validacioFirmaErrorMsg}"/>"></span></c:if></td>
										...
										<td><c:if test="${not empty annex.annexArxiuEstat}">${annex.annexArxiuEstat}</c:if></td>
										<td>
 							...
 							<div id="collapse-annex-${status.index}" class="panel-collapse collapse collapse-annex" role="tabpanel" aria-labelledby="dadesAnnex${status.index}" data-registre-id="${registre.id}"  data-fitxer-arxiu-uuid="${annex.uuid}">
								...
								<tbody>														
									...
									<tr>
										...
										<td>
											${annex.titol} - ${annex.nom} (<span title="${annex.tamany} bytes">${annex.tamanyStr}</span>)
											<a href="descarregarAnnex/${annex.id}?versioImprimible=true" class="btn btn-default btn-sm pull-right">
												<span class="fa fa-download" title="<spring:message code="registre.annex.detalls.camp.fitxer.descarregar"/>"></span>
											</a>
										</td>
									</tr>
