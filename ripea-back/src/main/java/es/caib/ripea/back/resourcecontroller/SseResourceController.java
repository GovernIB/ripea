...
//            		.id(String.valueOf(System.currentTimeMillis())));
     ...
     *
     * El recàlcul (eventService.getValidacionsInicialsExpedient) es fa des d'un fil de @JmsListener sense petició
     * web; el mètode del EJB està marcat com @PermitAll perquè la seguretat del contenidor no bloquegi la crida.
     */
    @Async
    ...
    public void handleEventErrorsValidacioEvict(ErrorsValidacioEvictEvent evictEvent) {
    	...
    	for (Long expedientId : evictEvent.getExpedientIds()) {
    		...
    		try {
    			...
    			logger.error("Error recalculant/notificant validacions per l'expedient " + expedientId, ex);
    		}
    	}
    }
