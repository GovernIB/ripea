...
public class ExpedientEstatServiceImpl implements ExpedientEstatService {
	...
	public ExpedientEstatDto findExpedientEstatById(
		...
		entityComprovarHelper.comprovarEntitat(
				...
				false,
				false, false, false);
		ExpedientEstatEntity estat =  expedientEstatRepository.getOne(id);
	...
	public ExpedientEstatDto updateExpedientEstat(
		...
		ExpedientEstatEntity expedientEstat = expedientEstatRepository.getOne(estat.getId());
	...
	public ExpedientEstatDto deleteExpedientEstat(
		...
		entityComprovarHelper.comprovarEntitatPerMetaExpedients(entitatId);
		ExpedientEstatEntity entity = expedientEstatRepository.getOne(expedientEstatId);
		if (!entity.getExpedients().isEmpty()) {
			throw new ValidationException("expedient.estat.controller.esborrat.error.restriccio1");
		}
		if (metaExpedientTascaRepository.countByEstatCrearTascaOrEstatFinalitzarTasca(entity, entity)>0) {
			throw new ValidationException("expedient.estat.controller.esborrat.error.restriccio2");
		}
