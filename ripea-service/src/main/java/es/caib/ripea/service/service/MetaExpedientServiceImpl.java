...
public class MetaExpedientServiceImpl implements MetaExpedientService {
	...
	public void createFromImport(Long entitatId, MetaExpedientExportDto procedimentImportat, String rolActual, Long organId) {
		...
		metaExpedientHelper.createFromImport(entitatId, procedimentImportat, rolActual, organId);
	}
	...
	public void updateFromImport(Long entitatId, MetaExpedientExportDto procedimentImportat, String rolActual, Long organId) {
		...
		metaExpedientHelper.updateFromImport(entitatId, procedimentImportat, rolActual, organId);
	}
	...
	public MetaExpedientDto getAndCheckAdminPermission(Long entitatId, Long id, Long organId) {
		logger.debug("Consulta del meta-expedient (" + "entitatId=" + entitatId + ", " + "id=" + id + ")");
		EntitatEntity entitat = entityComprovarHelper.comprovarEntitatPerMetaExpedients(entitatId);
		MetaExpedientEntity metaExpedient = entityComprovarHelper.comprovarAccesMetaExpedient(entitat, id, organId, false);
		MetaExpedientDto resposta = conversioTipusHelper.convertir(metaExpedient, MetaExpedientDto.class);
		if (resposta != null) {
			metaNodeHelper.omplirMetaDadesPerMetaNode(resposta);
			metaExpedientHelper.omplirMetaDocumentsPerMetaExpedient(metaExpedient, resposta);
	...
	public MetaExpedientDto getAndCheckOrganPermission(Long entitatId, Long id, OrganGestorDto organActual, boolean inclouRolDissenyador) {
		logger.debug("Consulta del meta-expedient (" + "entitatId=" + entitatId + ", " + "id=" + id + ")");
		EntitatEntity entitat = entityComprovarHelper.comprovarEntitatPerMetaExpedients(entitatId);
		MetaExpedientEntity metaExpedient = entityComprovarHelper.comprovarAccesMetaExpedient(entitat, id, organActual.getId(), inclouRolDissenyador);
		MetaExpedientDto resposta = conversioTipusHelper.convertir(metaExpedient, MetaExpedientDto.class);
		if (resposta != null) {
			metaNodeHelper.omplirMetaDadesPerMetaNode(resposta);
			metaExpedientHelper.omplirMetaDocumentsPerMetaExpedient(metaExpedient, resposta);
	...
	private PaginaDto<MetaExpedientDto> findByOrganGestor(
			...
			PaginacioParamsDto paginacioParams) {
		return paginacioHelper.toPaginaDto(
				findByOrganGestorEntity(entitatId, organGestorId, hasPermisAdmComu, filtre, paginacioParams),
				MetaExpedientDto.class,
				organGestorId,
	...
	public MetaExpedientTascaDto tascaUpdate(
		...
		if (metaExpedientTasca.getEstatIdCrearTasca() != null) {
			estatCrearTasca = expedientEstatRepository.getOne(metaExpedientTasca.getEstatIdCrearTasca());
		}
		ExpedientEstatEntity estatFinalitzarTasca = null;
		if (metaExpedientTasca.getEstatIdFinalitzarTasca() != null) {
			estatFinalitzarTasca = expedientEstatRepository.getOne(metaExpedientTasca.getEstatIdFinalitzarTasca());
		}
	...
	public void actualitzaProcediments(
		...
		if (isRolFiltreOrgan) {
			...
			Page<MetaExpedientEntity> resultat = findByOrganGestorEntity(
					entitat.getId(),
					organActual==null?null:organActual.getId(),
					hasPermisAdmComu,
					new MetaExpedientFiltreDto(),
					sensePaginacio);
			if (resultat!=null) { metaExpedients = resultat.getContent(); }
		} else {
	...
	public boolean createValidacioTasca(MetaExpedientTascaValidacioDto metaExpedientTascaValidacioDto) {
		...
		if (repetits==null || repetits.size()==0) {
			...
			novaValidacio.setMetaExpedientTasca(metaExpedientTascaRepository.getOne(metaExpedientTascaValidacioDto.getMetaExpedientTasca().getId()));
	...
	public MetaExpedientTascaValidacioDto updateValidacioTasca(Long metaExpedientTascaValidacioId, String accio) {
		MetaExpedientTascaValidacioEntity aux = metaExpedientTascaValidacioRepository.getOne(metaExpedientTascaValidacioId);
