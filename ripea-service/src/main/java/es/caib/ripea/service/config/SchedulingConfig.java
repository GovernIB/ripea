...
public class SchedulingConfig implements SchedulingConfigurer {
    ...
    public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
        ...
        addTask(
                codiEsborrarIntegracionsAntigues,
                new Runnable() {
                    @SneakyThrows
                    @Override
                    public void run() {
                        monitorTasquesService.inici(codiEsborrarIntegracionsAntigues);
                        try {
                            createAuthenticationContext();
                            int esborrats = integracioHelper.esborrarAccionsMesAntigues3Mesos();
                            log.info("Esborrades {} accions d'integració amb més de 3 mesos d'antiguitat", esborrats);
                            monitorTasquesService.fi(codiEsborrarIntegracionsAntigues);
                        } catch (Throwable th) {
                            tractarErrorTascaSegonPla(th, codiEsborrarIntegracionsAntigues);
                        } finally {
                            SecurityContextHolder.clearContext();
                        }
                    }
        ...
        if (taskCodi.equals(codiTancarExpedientsEnArxiu)) {
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    ...
                    try {
                        String cron = configHelper.getConfig(PropertyConfig.TANCAMENT_LOGIC_CRON);
                        if (cron == null)
                            cron = "0 0 20 * * *";
            ...
            return new Trigger() {
                @Override
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    String cron = configHelper.getConfig(PropertyConfig.CONSULTA_CANVIS_ORGANIGRAMA);
                    if (cron == null)
                        cron = "0 45 2 * * *";
            ...
            return new Trigger() {
                @Override
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    String cron = configHelper.getConfig(PropertyConfig.ACTUALITZAR_PROCEDIMENTS);
                    if (cron == null)
                        cron = "0 15 * * * *";
                    CronTrigger trigger = new CronTrigger(cron);
                    Date nextExecution = trigger.nextExecutionTime(triggerContext);
            ...
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    ...
                    try {
                        trigger = new PeriodicTrigger(configHelper.getAsLong(PropertyConfig.GUARDAR_ARXIU_INTERESSATS), TimeUnit.MILLISECONDS);
                    } catch (Exception e) {
            ...
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    PeriodicTrigger trigger = null;
                    try {
                        trigger = new PeriodicTrigger(configHelper.getAsLong(PropertyConfig.GUARDAR_ARXIU_CONTINGUTS_PENDENTS), TimeUnit.MILLISECONDS);
                    } catch (Exception e) {
            ...
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    CronTrigger trigger = null;
                    try {
                        trigger = new CronTrigger(configHelper.getConfig(PropertyConfig.ENVIAR_EMAILS_PENDENTS_AGRUPATS_CRON));
                    } catch (Exception e) {
            ...
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    CronTrigger trigger = null;
                    try {
                        trigger = new CronTrigger(configHelper.getConfig(PropertyConfig.ENVIAR_EMAILS_PENDENTS_PROCEDIMENT_COMENTARI_CRON));
                    } catch (Exception e) {
            ...
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    PeriodicTrigger trigger = null;
                    try {
                        trigger = new PeriodicTrigger(configHelper.getAsLong(PropertyConfig.BUIDAR_CACHES_DOMINIS_RATE), TimeUnit.MILLISECONDS);
                    } catch (Exception e) {
...
//        				monitorTasquesService.updateProperaExecucio(codiBuidarCachesDominis, longNextExecution);
            ...
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    PeriodicTrigger trigger = null;
                    try {
                        trigger = new PeriodicTrigger(configHelper.getAsLong(PropertyConfig.REINTENTAR_CANVI_ESTAT_DISTRIBUCIO), TimeUnit.MILLISECONDS);
                    } catch (Exception e) {
                        log.error("Error getting next execution date for buidarCacheDominis()", e);
                    }
                    trigger.setFixedRate(true);
            ...
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    PeriodicTrigger trigger = null;
                    try {
                        trigger = new PeriodicTrigger(configHelper.getAsLong(PropertyConfig.PROCESSAR_ANOTACIONS_PETICIONS_PENDENTS_RATE), TimeUnit.MILLISECONDS);
                    } catch (Exception e) {
            ...
            return new Trigger() {
                ...
                public Date nextExecutionTime(TriggerContext triggerContext) {
                    PeriodicTrigger trigger = null;
                    try {
                        trigger = new PeriodicTrigger(configHelper.getAsLong(PropertyConfig.EXECUTAR_EXECUCIONS_MASSIVES_RATE), TimeUnit.MILLISECONDS);
                    } catch (Exception e) {
        ...
        } else if (taskCodi.equals(codiEsborrarIntegracionsAntigues)) {
            return new Trigger() {
                @Override
                public Date nextExecutionTime(TriggerContext triggerContext) {
