package es.caib.ripea.persistence.repository.config;
...
public interface ConfigRepository extends JpaRepository<ConfigEntity, String> {
    ...
    @Query("SELECT c.key FROM ConfigEntity c WHERE c.entitatCodi IS NOT NULL")
    List<String> findKeysEntitatCodiNotNull();
