...
public interface MetaDocumentRepository extends JpaRepository<MetaDocumentEntity, Long> {
	...
	@Query(	"from " +
			...
			"and (md.portafirmesResponsables != null or EXISTS (SELECT f FROM MetaDocumentFluxPortafibEntity f WHERE f.metaDocument = md))")
	...
	@Query("select md.id from MetaDocumentEntity md where md.portafirmesFluxId is not null order by md.id asc")
	List<Long> findWhereFluxNotNull();
}