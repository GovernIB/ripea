...
public class PaginacioHelper {
	...
	public <T> PaginaDto<T> prepararPagina(PaginaDto<T> pagina, List<List<T>> pagines, List<T> llista) {
		pagina.setElementsTotal(llista.size());
		pagina.setPrimera(pagina.getNumero() == 0);
		pagina.setDarrera(pagina.getNumero() == pagines.size() -1);
		pagina.setPosteriors(!pagina.isDarrera());
		pagina.setAnteriors(!pagina.isPrimera());
		return pagina;
	}
