...
import es.caib.ripea.plugin.digitalitzacio.DigitalitzacioErrorTipus;
		...
		try {
			...
			switch (status) {
				case ScanWebSimpleStatus.STATUS_REQUESTED_ID:
					{
						...
						resposta.setErrorDescripcio("S'ha rebut un estat inconsistent del procés"
				...
				case ScanWebSimpleStatus.STATUS_FINAL_ERROR:
					{
						resposta.setError(true);
						resposta.setErrorTipus(DigitalitzacioErrorTipus.FINAL_ERROR);
						resposta.setErrorDescripcio("Error durant la realització de l'escaneig/còpia autèntica: "
				...
				case ScanWebSimpleStatus.STATUS_CANCELLED:
					{
						resposta.setError(true);
						resposta.setErrorTipus(DigitalitzacioErrorTipus.CANCELLED);
						logger.error("Durant el procés, l'usuari ha cancelat la transacció.");
						return resposta;
					}
				case ScanWebSimpleStatus.STATUS_FINAL_OK:
					{
						...
						if (result.getScannedFile() != null) {
							...
							logger.debug("La recuperació del fitxer escanejat s'ha realitzat amb èxit.");
						}
						...
						if (result.getSignedFile() != null) {
							...
							logger.debug("La recuperació del fitxer escanejat i firmat s'ha realitzat amb èxit.");
						}
					}
					break;
				default: {
